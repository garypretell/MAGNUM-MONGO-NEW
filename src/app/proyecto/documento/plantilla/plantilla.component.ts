...
export class PlantillaComponent implements OnInit, OnDestroy {
      ...
      dragulaService
        ...
        .subscribe(
          async ({ el, target, source, sourceModel, targetModel, item }) => {
            ...
            await this.templateService.actualizarTemplate(this.campos._id, data);
          }
        )
    );
  }
    ...
    this.sub = this.activatedroute.paramMap
      .pipe(
        map(async (params) => {
          ...
          this.documento = await this.documentoService.findDocumentobyId(
            this.docId
          );
          this.campos = await this.templateService.findTemplatebyDocumento(
            this.docId.toString()
          );
  ...
  async itemListSave() {
    ...
    await this.templateService.crearPlantilla({
      documento: this.docId.toString(),
      proyecto: this.proyId.toString(),
      campos: items,
    });
    await this.documentoService.updateDocumento(this.docId, {
      plantilla: true,
    });
    await this.documentoService.removeAfterActualizar(this.docId);
    this.campos = await this.templateService.findTemplatebyDocumento(
      this.docId.toString()
    );
    this.documento = await this.documentoService.findDocumentobyId(this.docId);
  }
  ...
  async addField() {
    ...
    temp.push(data);
    await this.templateService.actualizarTemplate(this.campos._id, {
      campos: temp,
    });
    this.campos = await this.templateService.findTemplatebyDocumento(
      this.docId.toString()
  ...
  async editField(field): Promise<any> {
    ...
    var doc: any = await this.templateService.findTemplatebyDocumento(
      this.docId.toString()
    );
